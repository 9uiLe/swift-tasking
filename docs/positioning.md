...
- 公開 API の表面積が小さく依存ゼロで、実装全体を読み切れる。
...
- 重複ポリシーの語彙(ignoreNew / cancelExisting / allowConcurrent)が
  レビューの共通言語になる。
- テストがポリシー・キャンセル・ライフタイム・終了待ちの契約を実行可能な仕様として固定している。
...
- **業務エラーは store が運ばない**: operation から漏れたエラーは、設定した
  `ViewTaskStore(onUnhandledError:)` で release ビルドでも観測できる。ただしこれは
  契約違反の最終通知点であり、回復経路ではない。handler 未設定時は互換性のため
  debug assertion／release 無通知の従来動作を維持する(ADR-0003)。
...
2026-07-03 のグリリングで、核心の一文(§1 = 所有者不在を根に)、appBound の推奨構成
([lifetimes.md](lifetimes.md))、公開ゴール(§6)、未処理エラーの観測方針(ADR-0003)、
名称・ライセンス・言語方針(ADR-0008: swift-tasking / MIT / 二層言語構造)は確定した。
残る事項:
